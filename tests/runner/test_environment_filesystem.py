...
async def test_filesystem_changes_tracking_git_workspace(tmp_path: Path) -> None:
    ...
    ws = tmp_path / "repo"
    ws.mkdir()
    # The minimal validated .git layout (HEAD/objects/refs) makes
    # _find_git_root classify this as a git workspace, so the runner builds a
    # GitFilesystemRegistry (complete tracking); git status accepts it too.
    git_dir = ws / ".git"
